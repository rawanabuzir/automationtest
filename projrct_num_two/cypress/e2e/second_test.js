...
describe('Add four items to cart then do the checkout ', () => {
    it('if checkout button is working correctlly', () => {
        ...
        cy.login('rawan', 'rawan123456789')
            //login or regester button existing on the header
        ...
        cy.get('.block_7 > .nav > .dropdown > .dropdown-toggle').click(); //to click on add to chart button
        cy.get('#cart_checkout1').click(); // to click on the checkout button
        cy.get('#accountFrm > fieldset > .btn').click(); //click on contenue button after choose the Register Account option from the Checkout Options card
